  ...
  return (
    <div className="flex flex-col gap-6">
      <form
        ...
        className="shadow-panel grid grid-cols-1 gap-4 rounded-xl border border-carbon-700 bg-carbon-800 p-6 md:grid-cols-4"
      >
        ...
        <div className="flex-1">
          <label className="text-sm font-medium text-gold-100/80">Categoria</label>
          <select
            value={categoriaId}
          ...
          <article key={a.telegramId} className="shadow-panel rounded-xl border border-carbon-700 bg-carbon-800 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-gold-100/70">Categoria: {nombreCategoria(a.categoriaId)}</p>
              </div>
              <span
            ...
            <select
              ...
              className="focus-ring mt-2 w-full rounded-md border border-carbon-600 bg-carbon-900 px-2.5 py-1.5 text-xs text-gold-50"
            >
              {categoriasActivas.map((categoria) => (
                <option key={categoria.id} value={categoria.id}>
                  {categoria.nombre} ({categoria.porcentajeComision}%)
      ...
      <div className="shadow-panel hidden overflow-x-auto rounded-xl border border-carbon-700 bg-carbon-800 md:block">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
              ...
              <tr key={a.telegramId} className="border-b border-carbon-700/50 text-gold-100/80">
                ...
                <td className="px-4 py-3">
                  <select
                    value={a.categoriaId}
                    onChange={(e) => actualizarCategoria(a, e.target.value)}
                    className="focus-ring rounded-md border border-carbon-600 bg-carbon-900 px-2.5 py-1.5 text-xs text-gold-50"
                  >
                    {categoriasActivas.map((categoria) => (
                      <option key={categoria.id} value={categoria.id}>
                        {categoria.nombre} ({categoria.porcentajeComision}%)
            ...
            {asesores.length === 0 && (
              <tr>
                <td colSpan={5} className="py-10 text-center text-gold-100/40">
                  Ningún asesor registrado todavía.
                </td>
              </tr>
            )}
