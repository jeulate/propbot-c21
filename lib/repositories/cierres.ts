...
import {
  estaDentroDelRango,
  obtenerRangoPeriodoBolivia,
  obtenerRangoPersonalizadoBolivia,
...
export async function crearCierre(
  ...
  if (!asesor || !asesor.activo) {
    throw new Error(
      "El asesor que registra el cierre no esta autorizado o esta inactivo.",
    );
  }
  ...
  if (!categoria || !categoria.activo) {
    throw new Error(
      "El asesor no tiene una categoria activa para calcular su comision.",
    );
  }
...
export async function listarTodosLosCierres(): Promise<Cierre[]> {
  const total = await kv.zcard(KEYS.cierresIndex);
  if (total === 0) return [];
  const ids = await kv.zrange<string[]>(KEYS.cierresIndex, 0, total - 1, {
    rev: true,
  });
  const cierres = await Promise.all(ids.map((id) => obtenerCierre(id)));
...
export async function obtenerMetricas(
  ...
  const totalComisiones = cierres.reduce(
    (acc, c) => acc + (c.montoComision || 0),
    0,
  );
  const totalPagosReales = cierres.reduce(
    (acc, c) => acc + (c.montoPagoRealAsesor || 0),
    0,
  );
  const totalTransacciones = cierres.reduce(
  ...
  function calcularAvanceObjetivo(objetivo: number) {
    const porcentaje =
      objetivo > 0 ? (comisionAnualOficina / objetivo) * 100 : 0;
  ...
  const captacionesPeriodo = await listarCaptacionesPorPeriodo(
    anioMeta,
    mesMeta,
  );
  ...
  const fechaMetaAnterior = new Date(
    Date.UTC(anioMeta, mesMeta - 2, 1, 4, 0, 0),
  );
  ...
  const metaAnterior = await obtenerMetaMensual(
    anioMetaAnterior,
    mesMetaAnterior,
  );
  ...
  const porcentajeMeta =
    objetivoMeta > 0
      ? Math.min((totalComisiones / objetivoMeta) * 100, 100)
      : 0;
  ...
  const metaMensual = {
    ...
    variacionObjetivoVsMesAnterior: calcularVariacion(
      objetivoMeta,
      objetivoMetaAnterior,
    ),
    configurada: !!metaActual,
  };

  const ticketPromedio =
    totalCierres > 0 ? totalTransacciones / totalCierres : 0;
  const comisionPromedio =
    totalCierres > 0 ? totalComisiones / totalCierres : 0;

  const porTipo: Record<string, number> = {};
  const porAsesor: Record<
    string,
    { nombre: string; cierres: number; comision: number }
  > = {};
  ...
  for (const c of cierres) {
    ...
    const captadorRegistrado = await obtenerAsesorRegistradoActivo(
      c.asesorCaptadorId,
    );
    if (captadorRegistrado) {
      sumarRanking(
        porCaptador,
        captadorRegistrado.telegramId,
        captadorRegistrado.nombre,
      );
    }

    const colocadorRegistrado = await obtenerAsesorRegistradoActivo(
      c.asesorColocadorId,
    );
    if (colocadorRegistrado) {
      sumarRanking(
        porColocador,
        colocadorRegistrado.telegramId,
        colocadorRegistrado.nombre,
      );
    }
  }
  ...
  const rankingCaptadores = captacionesPeriodo
    .map((item) => ({
